...
class Player(pygame.sprite.Sprite):
	...
	def shoot(self,mousePos):
		...
		if self.state == RIGHT:
			...
			pass
		shoot_sound.play()
		b = Bullet(PLAYER_POSX+shift_x,self.pos.y+shift_y,speedx,speedy)
		return b
...
class HUD(pygame.sprite.Sprite):
	...
	def drawHealth(self,health):
		if health > PLAYER_HEALTH - 5:
			text = self.font.render("Здоровье: "+str(health), 1,DARK_GREEN)
		elif health > 10:
			text = self.font.render("Здоровье: "+str(health), 1,DARK_YELLOW)
		else:
			text = self.font.render("Здоровье: "+str(health), 1,RED)
	...
	def drawBlink(self,retract):
		...
		if retractPerc == '100':
			retractPerc = "ГОТОВ"
			text = self.font.render("Скачок: "+retractPerc, 1,DARK_GREEN)
		else:
			text = self.font.render("Скачок: "+retractPerc, 1,RED)
		textPos = text.get_rect()
		textPos.centerx = 410
		self.surface.blit(text,textPos)
		self.image = self.surface
