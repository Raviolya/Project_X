...
class Game:
	...
	def show_game_over_screen(self):
		text_to_display = "Игра окончена"
		if not self.bosses:
			text_to_display = "Уровень зачищен ("+str(int(self.time/FPS))+" с)"
		pygame.draw.rect(self.screen,WHITE,(WIDTH/2, HEIGHT/2 - 35, 300,50))
		self.draw_text(text_to_display, 21, WIDTH/2 + 100, HEIGHT/2 - 20)
		self.draw_text("R для рестарта, Q для выхода",25,WIDTH/2 + 150, HEIGHT/ 2)
